# -*- coding: utf-8 -*-
"""
Created on Fri Nov 20 01:44:58 2020
...
def spread_information(result_dict,edges_dict,start_point_id, propagation_matrix, start_type, depth):
    
    linked_points=from_link_id_list_to_array(get_linked_id_to_point(edges_dict, start_point_id, -1, depth), depth)
    result_dict=update_dict(linked_points, propagation_matrix, result_dict, start_type)
...
def update_dict(link_array, propagation_matrix, result_dict, start_type):
...
def get_infos_back_to_point(result_dict,edges_dict,start_point_id, passage_matrix, depth, still_iter):
    
    still_iter=min(still_iter, depth)
    link_neigbhours=get_linked_type_to_point(result_dict,edges_dict,start_point_id, -1, still_iter)
...
def get_not_doable_points(result_dict,edges_dict, depth, still_iter):
    
    not_doable_points=[]
    still_iter=min(still_iter, depth)
    for id in result_dict.keys():
        if result_dict.get(id)==[0,0,0,0] and edges_dict.get(id)!=None:
            link_neigbhours=get_linked_type_to_point(result_dict,edges_dict,id, -1, still_iter)
...
def get_linked_type_to_point(result_dict,edges_dict,start_point_id, last_point, still_iter):
    ...
    if still_iter>0 and edges_dict.get(start_point_id)!=None:
        for reach_point_id in edges_dict.get(start_point_id):
            if reach_point_id!=last_point:
                ...
                new_val=get_linked_type_to_point(result_dict,edges_dict,reach_point_id, new_last_point, still_iter-1)
                if new_val!=[]:
                    for item in new_val:
...
def assign_point_type(df_test_target,df_train_target, df_edges, edges_dict_test, passage_matrix,pct_point, nb_reinforcment, depth,prct_max_reinforced):
    ...
    result_dict=initialyze_dict(df_test_target, len(passage_matrix[0]))
    type_dict, nb_type=get_type_dict(df_train_target)
    edges_dict=add_start_point_to_edge_dict(edges_dict_test, df_edges, starts_points_id)
    result_dict=spread_known_infos(df_train_target,starts_points_id, nb_type,result_dict,edges_dict,depth,type_dict,passage_matrix )
    result_dict,labeled_dict, not_doable_points=reinforced(edges_dict, depth,result_dict, type_dict,passage_matrix,df_target,prct_max_reinforced, nb_reinforcment=nb_reinforcment)
    return result_dict,labeled_dict,not_doable_points
    
def spread_known_infos(df_train_target,starts_points_id, nb_type,result_dict,edges_dict,depth,type_dict,passage_matrix ):
    ...
    for start in range(len(starts_points_id)):
        ...
        start_type=type_dict.get(df_train_target[df_train_target['id']==start_point_id]['page_type'].values[0])
        #print("start point "+str(start))
        result_dict= spread_information(result_dict,edges_dict,start_point_id, propagation_matrix, start_type, depth)
    result_dict=get_sum_back_to_prob(result_dict, nb_type)
    return result_dict
    
def reinforced(edges_dict, depth,result_dict, type_dict,passage_matrix,df_target,prct_max_reinforced, nb_reinforcment=6):
    for reinforcement in range(nb_reinforcment):
        not_doable_points=get_not_doable_points(result_dict,edges_dict, depth, depth)
        ...
        for start in range(nb_point_per_reinforcment):
            start_point_id=starts_points_id[start]
            #start_type=result_dict.get(start_point_id)
            result_dict=get_infos_back_to_point(result_dict, edges_dict, start_point_id, passage_matrix,depth, depth )
            
#        Y_real, Y_pred, nf_reinforced=get_pred_vs_Y_for_found(relabel_results(result_dict, type_dict), df_target)
#        accuracy_reinforced=metrics.accuracy_score(Y_real,Y_pred)
#        print(accuracy_reinforced)
#        print(nf_reinforced)
...
def parameters_research(df_train_target, df_edges,pct_point, depth_list, nb_reinforcment_list,prct_max_reinforced_list, nb_splits ):
        ...
        for depth in depth_list:
            ...
            for nb_reinforcment in nb_reinforcment_list:
                ...
                for prct_max_reinforced in prct_max_reinforced_list:
                    raw_dict, predict_dict,not_doable_points =assign_point_type(df_train_test_target,df_train_train_target, df_edges, edges_dict_test, passage_matrix,pct_point, nb_reinforcment, depth,prct_max_reinforced)
                    Y_real, Y_pred, nf=get_pred_vs_Y_for_found(predict_dict, df_target)
                    accuracy=metrics.accuracy_score(Y_real,Y_pred)
                    score=accuracy*(1-nf/len(test_target))
                    if score>best_score[-1]:
                        best_nb_node_depth_fold=depth
                        best_nb_reinforcment_fold=nb_reinforcment
                        best_prct_max_reinforced_fold=prct_max_reinforced
                        best_score[-1]=score
...
for pct_used in [0.05,0.1,0.2,0.5,0.6]:
    pct_point=pct_used
    print("results for "+str(pct_used*100)+"% of datas known in trains used for prediction")
    best_nb_node_depth,best_nb_reinforcment,best_prct_max_reinforced=parameters_research(df_train_target, df_edges,pct_point, depth_list, nb_reinforcment_list,prct_max_reinforced_list,5 )
    print("node depth selected: "+str(best_nb_node_depth)+" , number of reinforcement selected: "+str(best_nb_reinforcment)+" and percentage reinforced: "+str(best_prct_max_reinforced))
    pct_point=pct_used/(1-test_size)
    
    passage_matrix=get_passage_probabilities(df_train_target, df_edges, 10,10,best_nb_node_depth)
    
    
    result_dict, labeled_dict, not_doable_points=assign_point_type(df_test_target,df_train_target, df_edges, edges_dict_test, passage_matrix,pct_point, best_nb_reinforcment, best_nb_node_depth,best_prct_max_reinforced)
    Y_real, Y_pred, nf=get_pred_vs_Y_for_found(labeled_dict, df_target)
    print(str((1-nf/len(result_dict))*100)+"% of points classified")
    print(str((len(not_doable_points)/len(result_dict))*100)+"% of points totaly unclassifiable")
