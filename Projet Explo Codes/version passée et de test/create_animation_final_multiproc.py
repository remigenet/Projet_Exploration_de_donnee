...
# -*- coding: utf-8 -*-
"""
Created on Sat Oct 31 17:36:28 2020
...
def save_graph(item):
    ii=item[0]
    iter_=item[1]
    ax.view_init(elev=10., azim=ii*4)
    if ii<10:
        plt.savefig("/tmp/rgenet/image_animation/imgax"+str(iter_)+"number00"+str(ii)+".png")
    elif ii<100:
        plt.savefig("/tmp/rgenet/image_animation/imgax"+str(iter_)+"number0"+str(ii)+".png")
    else:
        plt.savefig("/tmp/rgenet/image_animation/imgax"+str(iter_)+"number"+str(ii)+".png")
...
for iter in range(len(my_point_list)):
    print(iter)
    t=[]
    point_size=[]
    point_part=[]
    for depth in range(iter+1):
    ...
    ax.scatter(xyz[:,0], xyz[:,1], xyz[:,2], s=point_size, c=color_map)
    list_start=[]
    list_reached=[]
    for start_node in t:
        t_edges=edge_dict.get(start_node)
    ...
    for i in range(my_edges.shape[0]):
        ax.plot(my_edges[i,0:2], my_edges[i,2:4],
                    my_edges[i,4:6],linewidth=edge_size_map[i], color=edge_color_map[i])#, color=(0.2, 1 - 0.1 * i, 0.8))
    if iter>0:
        img_list=[(i,iter) for i in range(90)]
        with Pool() as p:
            p.map(save_graph, img_list)
    else:
        for ii in range(0,60,1):
            plt.savefig("/tmp/rgenet/image_animation/imgax"+str(iter)+"number00"+str(ii)+".png")
