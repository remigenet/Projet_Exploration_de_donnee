...
# -*- coding: utf-8 -*-
"""
Created on Thu Oct 29 17:26:35 2020
...
def create_X_reduced(df_target,page_types, df_know_infos, df_edges,X_ids, partition_dict ):
    
    nb_page_type=len(page_types)
    nb_ids=len(X_ids)
    X=np.zeros((nb_ids, nb_page_type*3+1))
    for iter in range(nb_ids):
        id=X_ids[iter]
        ...
        row_values=[partition_dict.get(id)]+list(direct_link.values())+list(second_link.values())+list(third_link.values())
        X[iter,:]=row_values
    return X
...
for iter in range(nb_iter):
    ...
    X_train=create_X_reduced(df_train_target, page_types, df_know_infos, df_edges,X_ids_train,partition_dict)
...
for decision_tree in my_decision_trees:
    ...
    X_test=create_X_reduced(df_test_target, page_types, df_know_infos, df_edges,X_ids_test,partition_dict)
