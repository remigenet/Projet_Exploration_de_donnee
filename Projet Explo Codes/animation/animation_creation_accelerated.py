...
# -*- coding: utf-8 -*-
"""
Created on Sat Oct 31 22:53:46 2020
...
for iter_2 in range(len(point_size)):
    point_size[iter_2]=7.5*point_size[iter_2]/np.sqrt(len(t))
...
while not_done: 
    print(iter)
    nb_points_added=min(point_part[min(iter, len(point_part)-1)], len(t)-iter-1)
    ...
    ax.view_init(elev=10., azim=angle*2)
    if iter<10:
...
video_name = 'my_animation_slow_realease.avi'
